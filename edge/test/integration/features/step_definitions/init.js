...
var environment = config['${projectName}-${teamName}-test_onboard6'].env;
...
console.log('&$${projectName}-${teamName}-test_onboard6 api: [' +  domains[environment] + ', ' + config['${projectName}-${teamName}-test_onboard6'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['${projectName}-${teamName}-test_onboard6'].app, config['${projectName}-${teamName}-test_onboard6'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['${projectName}-${teamName}-test_onboard6'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
